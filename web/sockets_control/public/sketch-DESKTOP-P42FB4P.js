...
const YAW_PULSE_MIN = -100;
const YAW_PULSE_MAX = 100;
const PITCH_PULSE_MIN = -50;
const PITCH_PULSE_MAX = 50;
...
function onMasterGo() {
    if (connectedSerial !== '') {
        for (var i = 0; i < vertices.length; i++) {
            var vec = vertices[i];
            var x = parseInt(map(vec.x, -1, 1, YAW_PULSE_MIN, YAW_PULSE_MAX));
            var y = parseInt(map(vec.y, -1, 1, PITCH_PULSE_MIN, PITCH_PULSE_MAX));
        ...
        console.log('No serial connected');
    }
}
